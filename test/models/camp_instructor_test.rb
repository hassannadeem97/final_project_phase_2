...
class CampInstructorTest < ActiveSupport::TestCase
  ...
  should validate_numericality_of(:camp_id).is_greater_than(0).only_integer
  should validate_numericality_of(:instructor_id).is_greater_than(0).only_integer
  should validate_uniqueness_of(:camp_id).scoped_to(:instructor_id)
  
  
   context "Creating a set of camp_instructors" do
    setup do
